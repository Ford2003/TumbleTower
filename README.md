...
In order to run your app, you will need to create a `.env` file. Rename the file [/examples/react-colyseus/example.env](/examples/react-colyseus/.env) to `.env` and fill it in with the appropriate OAuth2 variables. The OAuth2 variables can be found in the OAuth2 tab on the developer portal, as described [here](https://discord.com/developers/docs/activities/building-an-activity#find-your-oauth2-credentials)
...
- Extend `PlayerMenu.ts`' schema to include x and y as numbers

```diff
--- a/examples/react-colyseus/packages/server/src/entities/PlayerMenu.ts
+++ b/examples/react-colyseus/packages/server/src/entities/PlayerMenu.ts
@@ -1,6 +1,6 @@
 import {Schema, type} from '@colyseus/schema';

-export type TPlayerOptions = Pick<PlayerMenu, 'sessionId' | 'userId' | 'name' | 'avatarUri' | 'talking'>;
+export type TPlayerOptions = Pick<PlayerMenu, 'sessionId' | 'userId' | 'name' | 'avatarUri' | 'talking' | 'x' | 'y'>;

 export class PlayerMenu extends Schema {
   @type('string')
@@ -18,6 +18,12 @@ export class PlayerMenu extends Schema {
   @type('boolean')
...
+
   ...
   constructor({name, userId, avatarUri, sessionId}: TPlayerOptions) {
     super();
@@ -25,5 +31,7 @@ export class PlayerMenu extends Schema {
...
```diff
--- a/examples/react-colyseus/packages/client/src/components/MainMenu.tsx
+++ b/examples/react-colyseus/packages/client/src/components/MainMenu.tsx
...
```diff
--- a/examples/react-colyseus/packages/client/src/components/PlayerMenu.tsx
+++ b/examples/react-colyseus/packages/client/src/components/PlayerMenu.tsx
@@ -2,13 +2,15 @@ import * as React from 'react';
 import {TPlayerOptions} from 'server/src/entities/PlayerMenu';
 import './PlayerMenu.css';

-export function PlayerMenu({avatarUri, name, talking}: TPlayerOptions) {
+export function PlayerMenu({avatarUri, name, talking, x, y}: TPlayerOptions) {
   return (
