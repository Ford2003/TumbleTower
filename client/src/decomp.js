...
exports.removeDuplicatePoints = exports.removeCollinearPoints = exports.quickDecomp = exports.isSimple = exports.decomp = exports.makeCCW = void 0;
/**
...
function polygonCanSee(polygon, a, b) {
    if (isLeftOn(polygonAt(polygon, a + 1), polygonAt(polygon, a), polygonAt(polygon, b)) &&
    ...
    for (let i = 0; i !== polygon.length; ++i) {
        ...
        if (isLeftOn(polygonAt(polygon, a), polygonAt(polygon, b), polygonAt(polygon, i + 1)) &&
            isRightOn(polygonAt(polygon, a), polygonAt(polygon, b), polygonAt(polygon, i))) {
            // if diag intersects an edge
            const l1 = [polygonAt(polygon, a), polygonAt(polygon, b)];
            const l2 = [polygonAt(polygon, i), polygonAt(polygon, i + 1)];
...
function polygonQuickDecomp(polygon, result, reflexVertices, steinerPoints, delta, maxlevel, level) {
    ...
    level++;
    if (level > maxlevel)
        throw new Error(`quickDecomp: max level (${maxlevel}) reached.`);
    for (let i = 0; i < polygon.length; ++i) {
        if (polygonIsReflex(poly, i)) {
            reflexVertices.push(poly[i]);
            upperDist = lowerDist = Number.MAX_VALUE;
            for (let j = 0; j < polygon.length; ++j) {
                if (isLeft(polygonAt(poly, i - 1), polygonAt(poly, i), polygonAt(poly, j)) &&
                ...
                if (isLeft(polygonAt(poly, i + 1), polygonAt(poly, i), polygonAt(poly, j + 1)) &&
...
function quickDecomp(polygon, delta = 0, maxlevel = 100) {
 ...
 */
function removeCollinearPoints(polygon, precision) {
    let num = 0;
